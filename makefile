...
PYTEST := pytest . --cov=. --cov-config=.coveragerc $(pytest_args) --capture=no
...
DEBUG_SET_ENV_VARS := \
	...
	export LOGOUT_REDIRECT_URL=http://buyer.trade.great.dev:8001; \
...
debug_test:
	$(DEBUG_SET_ENV_VARS) && $(FLAKE8) && $(COLLECT_STATIC) && $(PYTEST)
