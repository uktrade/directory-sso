...
def test_get_user_by_email_with_enabled_test_api(client, active_user):
    response = client.get(reverse('testapi:user_by_email', kwargs={'email': active_user.email}))
    assert response.status_code == status.HTTP_200_OK


@pytest.mark.django_db
def test_should_get_sso_id_and_is_verified_flag_for_active_user(client, active_user):
    response = client.get(reverse('testapi:user_by_email', kwargs={'email': active_user.email}))
    assert response.data == {'is_verified': True, 'sso_id': 11}


@pytest.mark.django_db
def test_should_get_sso_id_and_is_verified_flag_for_inactive_user(client, inactive_user):
    response = client.get(reverse('testapi:user_by_email', kwargs={'email': inactive_user.email}))
...
def test_should_get_404_for_non_existing_user(client):
    ...
    response = client.get(reverse('testapi:user_by_email', kwargs=kwargs))
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.django_db
def test_should_get_404_when_email_is_not_present(client):
    response = client.get(reverse('testapi:user_by_email', kwargs={'email': ''}))
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.django_db
def test_should_get_404_when_email_is_none(client):
    response = client.get(reverse('testapi:user_by_email', kwargs={'email': None}))
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.django_db
def test_get_user_by_email_with_disabled_test_api(client, settings, active_user):
...
def test_delete_user_by_email_with_enabled_test_api(client, active_user):
    response = client.delete(reverse('testapi:user_by_email', kwargs={'email': active_user.email}))
...
def test_delete_non_existing_user_should_get_404(client):
    kwargs = {'email': 'non_existing@user.com'}
    response = client.delete(reverse('testapi:user_by_email', kwargs=kwargs))
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.django_db
def test_delete_user_should_get_404_when_email_is_not_present(client):
    response = client.delete(reverse('testapi:user_by_email', kwargs={'email': ''}))
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.django_db
def test_delete_user_should_get_404_when_email_is_none(client):
    response = client.delete(reverse('testapi:user_by_email', kwargs={'email': None}))
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.django_db
def test_delete_user_by_email_with_disabled_test_api(client, settings, active_user):
    settings.FEATURE_FLAGS = {**settings.FEATURE_FLAGS, 'TEST_API_ON': False}
    response = client.delete(reverse('testapi:user_by_email', kwargs={'email': active_user.email}))
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_patch_non_existing_user_should_get_404(client, data):
    kwargs = {'email': 'non_existing@user.com'}
    response = client.patch(reverse('testapi:user_by_email', kwargs=kwargs), data=data)
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_patch_user_should_get_404_when_email_is_not_present(client, data):
    response = client.patch(reverse('testapi:user_by_email', kwargs={'email': ''}), data=data)
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_patch_user_should_get_404_when_email_is_none(client, data):
    response = client.patch(reverse('testapi:user_by_email', kwargs={'email': None}), data=data)
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
@pytest.mark.django_db
def test_patch_user_by_email_with_disabled_test_api(client, settings, active_user, data):
    settings.FEATURE_FLAGS = {**settings.FEATURE_FLAGS, 'TEST_API_ON': False}
    response = client.patch(reverse('testapi:user_by_email', kwargs={'email': active_user.email}), data=data)
    assert response.status_code == status.HTTP_404_NOT_FOUND
