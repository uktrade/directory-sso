...
@pytest.mark.django_db
@patch('config.signature.SignatureCheckPermission.has_permission', Mock)
@patch.object(SessionUserAPIView, 'get_object')
def test_get_session_user_cached_response(mock_get_object):
    user, user_session = setup_data()
...
@patch('config.signature.SignatureCheckPermission.has_permission', Mock)
@patch.object(SessionUserAPIView, 'get_object')
def test_get_session_user_cached_response_multiple_users(mock_get_object):
    client = APIClient()
    user_session_groups = [
    ...
    assert mock_get_object.call_count == 3
